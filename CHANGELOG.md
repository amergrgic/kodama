...
- **Fresh install told users to run `kodama` before the alias was active** — the installer adds the `kodama` alias to your shell profile, but the alias isn't available in the terminal that ran the install until the profile is re-sourced (a child process cannot reload its parent shell). The "Done" summary now prints an explicit activation step (`source <profile>`) whenever a new alias was added, and notes that new terminals pick it up automatically.
...
- Crew dashboard UI page: version info, getting started instructions, agent roster
- Sidebar icon (tree emoji via `iconUrl`)
...
- `.kiro/` walk-up no longer matches the global `~/.kiro/` directory — previously caused all projects without git to resolve to HOME
...
- Orchestrator now MUST write memory after first project discovery (previously skipped silently)
- Landing page updated: OG meta tags for Slack previews, CLI/memory sections, install refresh
- Dropped update cache — checks GitHub on every session start
...
[0.9.2]: https://github.com/amergrgic/kodama/releases/tag/v0.9.2
